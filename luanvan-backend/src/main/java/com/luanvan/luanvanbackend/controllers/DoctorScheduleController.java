...
public class DoctorScheduleController {
    ...
    public ResponseEntity<Map<String, Object>> createBulkSlotsFromWorkShifts(
        ...
        try {
            ...
            // If overwrite, delete existing slots first (to be implemented in service)
            if (overwrite) {
                try {
                    // TODO: Implement deleteAutoGeneratedSlotsByDoctorSpecialtyAndDateRange in service
                    log.info("Overwrite requested for doctor {} in date range {}-{}", 
                             doctor.getDoctorId(), startDate, endDate);
                } catch (Exception e) {
                    log.warn("Failed to delete existing slots: {}", e.getMessage());
                }
            }
                        ...
                        while (slotStartTime.isBefore(shift.getEndTime())) {
                            ...
                            try {
                                ...
                                AvailabilitySlotDTO slotDTO = new AvailabilitySlotDTO(
                                    ...
                                    slotStartTime,
                                    slotEndTime,
                                    "CANCELLED_BY_CLINIC", // Default disabled
                                    clinicId
                                );
